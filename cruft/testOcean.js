...
describe('Ocean Interface', function oceanInterface() {
  ...
  describe('Try to ask for a pretty list of drops', function prettyDropTest() {
    ...
    it('should have no drops with an odd tag', function (done) {
      const p = ocean.prettyListOfDrops('FOO9876');
      p.should.eventually.match('No drops');
    });
  });

  describe('Try to wait for a pretty list of drops', function () {
    it('should eventually get a list of drops', function (done) {
      this.timeout(60000);
      pro().then(ocean.allActionsComplete())
      .then(ocean.prettyListOfDrops())
